    ...
    def run(self, args):
        ...
        if data:
            ...
            for x in data:
                ...
                final_data[0][12] += x[12] #91-Above

        """final_data = []
        size_data = 0
        if data:
            size_data = len(data)
            final_data.append(data[size_data-1])"""

        return columns, final_data, None, chart

    def get_columns(self, party_naming_by, args):
        columns = [_("Posting Date**") + ":Date:80",
                   _(args.get("party_type")+ "**") + ":Link/" + args.get("party_type") + ":200"]
        ...
        for label in ("0-{range1}**".format(range1=self.filters["range1"]),
                      "{range1}-{range2}**".format(range1=cint(self.filters["range1"]) + 1,
                                                 range2=self.filters["range2"]),
                      "{range2}-{range3}**".format(range2=cint(self.filters["range2"]) + 1,
        ...
        for gle in self.get_entries_till(self.filters.report_date, args.get("party_type")):
            if self.is_receivable_or_payable(gle, dr_or_cr, future_vouchers):
                outstanding_amount = flt(self.get_outstanding_amount(gle,
                                                                     self.filters.report_date, dr_or_cr),
                                         currency_precision)
        ...
        if not hasattr(self, "party_map"):
            if party_type == "Customer":
                self.party_map = dict(((r.name, r) for r in frappe.db.sql("""select {0}, {1}, {2} from `tab{3}`"""
                                                                          .format("name", "customer_name", "territory",
                                                                                  party_type), as_dict=True)))

            elif party_type == "Supplier":
                self.party_map = dict(((r.name, r) for r in frappe.db.sql("""select {0}, {1}, {2} from `tab{3}`"""
                                                                          .format("name", "supplier_name",
